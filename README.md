...
Архитектура проекта зафиксирована отдельными правилами и проверяется как часть рабочего процесса. Главная цель этих правил — не дать MCP-серверу незаметно превратиться в небезопасную обёртку над аккаунтом Telegram.

Ключевые инварианты:

- локальная сессия Telegram — явная доверенная граница проекта;
- все инструменты MCP работают только на чтение, пока отдельная будущая спецификация явно не добавит команды;
- конфигурация, авторизация и готовность сессии проверяются до публикации инструментов;
- внешние входные данные валидируются до обращения к Telegram;
- ошибки мапятся в типизированные ошибки приложения, а не прячутся за пустыми результатами;
- DTO и refs между инструментами остаются стабильными контрактами;
- код разделён по понятным продуктовым операциям: list folders, resolve chat, search messages, get context и так далее;
- новые Telegram tool/query-сценарии не складываются в `helpers`, `utils`, `common`, `misc` или широкие файлы-свалки.
