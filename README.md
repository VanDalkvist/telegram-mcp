...
- Точка сборки приложения загружает `.env`, открывает хранилище сессии, создаёт GramJS client и проверяет авторизацию в Telegram до публикации tools.
- Слой MCP-интерфейса регистрирует tools, валидирует входные данные и превращает ошибки в публичный MCP-ответ.
- Прикладной слой задаёт контракт Telegram queries: какие read-only операции существуют и какие DTO они возвращают.
- Инфраструктурный слой работает с файловой сессией, логами, GramJS client и конкретными запросами к Telegram.
- Доменный слой хранит типы, refs, правила для окон дат и типизированные ошибки.
...
Архитектурные правила проекта фиксируют границы этой схемы: локальная сессия Telegram считается доверенной границей, tools остаются только на чтение, config/auth/session проверяются до старта, внешние inputs валидируются до Telegram, ошибки не прячутся за пустыми результатами, а новые Telegram tool/query-сценарии добавляются отдельными модулями по смыслу операции.
